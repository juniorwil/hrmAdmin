      ...
      if($this->getRequest()->isPost()) 
      {
        ...
        if ($request->isPost()) 
            {   
              ...
              (
                 "titulo"    =>  'Opciones del rol',
                 "datos"     =>  $d->getGeneral("select a.id as idM2, a.nombre as nomM2,
                              case when c.nuevo = 1 then 1 else 0 end as nuevo,
                              case when c.modificar = 1 then 1 else 0 end as modificar,
                              case when c.eliminar = 1 then 1 else 0 end as eliminar,
                              case when c.aprobar = 1 then 1 else 0 end as aprobar
                              from c_mu2 a 
                              inner join c_mu1 b on a.idM1=b.id
                              left join c_roles_o c on c.idM3=a.id
                              where a.idM1=".$id3." order by a.nombre"),// Listado de formularios            
